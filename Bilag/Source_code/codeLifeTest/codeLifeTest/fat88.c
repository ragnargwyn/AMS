...
//Modified: Thomas Sanberg Jensen 31-05-2018
...
char SD_init() {
	...
	for(i=0; i<10 && SD_command(0x40, 0x00000000, 0x95, 8) != 1; i++) //sends cmd0
		_delay_ms(100);
	...
	for(i=0; i<10 && SD_command(0x41, 0x00000000, 0xFF, 8) != 0; i++) //sends cmd1
		_delay_ms(100);

	if(i == 10) // card did not come out of idle
		return -2;
		
	// CMD8 until card comes out of idle, but maximum of 10 times
	SD_command(0x48, 0x000001aa, 0x87, 8); //cmd8
	_delay_ms(100);

	//if(i == 10) // card did not come out of idle
	//return -2;
		
	// CMD58 until card comes out of idle, but maximum of 10 times
	//SD_command(0x7a, 0x00000000, 0xff, 8);
	...
	//acmd41
	//SD_command(0x68, 0x00000000, 0xFF, 8);
...
int main(int argc, char *argv[]) {
	...
	USARTInit(103); // 16 MHz / (16 * 9600 baud) - 1 = 103,smthng
    ...
    if(ret = fat16_open_file("WOOD", "TXT")) {
		uwrite_str("Open: ");
		uwrite_hex(ret);
		return -1;
	}
	
	uwrite_str("Open but what then?");
	...
	uwrite_str("shouldn't be more left");
